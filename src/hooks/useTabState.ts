...
interface UseTabStateReturn {
  ...
  createTasksTab: (projectPath: string, projectName?: string) => string | null;
...
export const useTabState = (): UseTabStateReturn => {
  ...
  const createTasksTab = useCallback((projectPath: string, projectName?: string): string | null => {
    // A board belongs to one project, so the tab is a singleton per path rather
    // than app-wide: two projects can have their boards open side by side.
    const existingTab = tabs.find(tab => tab.type === 'tasks' && tab.projectPath === projectPath);
    if (existingTab) {
      setActiveTab(existingTab.id);
      return existingTab.id;
    }

    const name = projectName || projectPath.split(/[/\\]/).filter(Boolean).pop() || 'Tasks';
