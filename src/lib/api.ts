 ...
 */
/** Kanban column a ticket sits in. */
...
/** Ticket priority, lowest first. */
export type TicketPriority = "low" | "medium" | "high" | "critical";

/** A board grouping tickets for one repository. */
export interface BoardProject {
  ...
  repo_path: string;
  /**
   * Shell command an agent's work must pass before a run counts as verified.
  ...
  auto_push: boolean;
  /**
   * Open a draft pull request for the pushed branch. Needs `auto_push`, since
   * there is nothing to open a request against otherwise.
   */
  open_pr: boolean;
  /**
   ...
   * pass, so the next ticket builds on it rather than starting where this one
   * did.
   */
...
export interface Ticket {
  ...
  /**
   * Runs started for this ticket. A queue gives up on one that keeps failing
   * rather than working through it forever.
   */
  attempts: number;
}
...
/** What a file would import, before anything is created. */
export interface ImportPreview {
  sheet: string;
  /** Headings that were understood. */
  columns: string[];
  /** Headings that were not — a misspelt one drops a whole column. */
...
export interface ImportResult {
  created: number;
  /** Rows left out, each carrying its reason. */
  skipped: ImportRow[];
}
...
/** One phase of an autonomous workflow run. */
...
/** Progress event emitted on `workflow-progress:<runId>`. */
export interface WorkflowEvent {
  ...
  message: string;
  /** Set once the run reaches a terminal phase. */
  ok: boolean | null;
}

/** A persisted autonomous workflow run. */
export interface WorkflowRun {
  ...
  completed_at: string | null;
  /**
   * How far the run has got. Kept on the run itself because progress events
   * only exist while something is listening — without it a panel opened
   * part-way through a run shows nothing and reads as stuck.
   */
...
export interface Preflight {
  is_git_repo: boolean;
  /**
   * Whether the repository has anything to branch from. A repository that has
   * only been initialised has not.
   */
  has_commits: boolean;
  is_dirty: boolean;
  /**
   * Paths in the way that are build output, and so safe to ignore. The agent's
   * own new files are untracked too, and ignoring those would keep real work
...
/** Options for starting an autonomous run. */
export interface StartWorkflowOptions {
  ...
  testCommand?: string;
  branchPrefix?: string;
  /** Retry attempts before giving up. Clamped to 1-10 by the backend. */
  maxIterations?: number;
  autoPush?: boolean;
  openPr?: boolean;
  mergeToMain?: boolean;
  /** Attach the run to a ticket so it drives that ticket's board column. */
  ticketId?: number;
}

export const api = {
  /**
   * Starts an autonomous run: branch, implement, test, retry, commit,
   * optionally push and open a draft PR.
   *
   * Returns immediately with the run id; follow progress on the
   * `workflow-progress:<runId>` event and raw agent output on
   * `workflow-output:<runId>`.
   */
  async startWorkflow(options: StartWorkflowOptions): Promise<number> {
    return await apiCall<number>("start_workflow", {
      ...
      mergeToMain: options.mergeToMain,
      ticketId: options.ticketId,
    });
  },

  /**
   * The ticket board for a project, created the first time it is asked for.
   * A board belongs to its project rather than being set up separately.
   */
  async boardForProject(repoPath: string, name?: string): Promise<BoardProject> {
    return await apiCall<BoardProject>("board_for_project", { repoPath, name });
  },

  /**
   * Edits a board. Omitted fields are left unchanged; passing an empty
   * `testCommand` clears it.
   */
  async updateBoardProject(input: {
    id: number;
    name?: string;
    repoPath?: string;
    testCommand?: string;
    autoPush?: boolean;
    /** Forced off by the backend when `autoPush` is turned off. */
    openPr?: boolean;
    autoApprove?: boolean;
    mergeToMain?: boolean;
  }): Promise<BoardProject> {
    return await apiCall<BoardProject>("update_board_project", input);
  },

  /** Lists the tickets on one board, in display order. */
  async listTickets(projectId: number): Promise<Ticket[]> {
    try {
      return await apiCall<Ticket[]>("list_tickets", { projectId });
    } catch (error) {
  ...
  /** Per-column ticket counts for one board. */
  async boardSummary(projectId: number): Promise<BoardSummary> {
    return await apiCall<BoardSummary>("board_summary", { projectId });
  },

  /**
   * Creates a ticket in the Pending column. The agent is chosen from the title
   * and description rather than passed in.
   */
  async createTicket(input: {
    ...
    return await apiCall<Ticket>("create_ticket", input);
  },

  /**
   * Edits a ticket. Omitted fields are left unchanged. Changing the title or
   * description picks the agent again, since it may be different work now.
   */
  async updateTicket(input: {
    ...
    return await apiCall<Ticket>("update_ticket", input);
  },

  /**
   * Moves a ticket between columns. The backend rejects transitions that would
   * skip the approval gate.
   */
  async setTicketStatus(id: number, status: TicketStatus): Promise<Ticket> {
    return await apiCall<Ticket>("set_ticket_status", { id, status });
  },

  /**
   * Writes the example sheet somewhere the person can open and edit it. The
   * template is compiled into the binary, so this works in a packaged app.
   */
  async saveTicketTemplate(filePath: string): Promise<void> {
    return await apiCall<void>("save_ticket_template", { filePath });
  },

  /**
   * Reads a spreadsheet and reports what it would import, creating nothing.
   * The first non-empty row is read as headings.
   */
  async previewTicketImport(filePath: string): Promise<ImportPreview> {
    return await apiCall<ImportPreview>("preview_ticket_import", { filePath });
  },

  /**
   * Creates tickets from a spreadsheet, skipping rows that cannot become one
   * and reporting them. Each ticket's agent is chosen from its own text.
   */
  async importTickets(input: {
    projectId: number;
    filePath: string;
  ...
  /**
   * Gives the tickets a queue has passed over another go. Returns how many
   * were reset.
   */
  async resetSkippedTickets(projectId: number): Promise<number> {
    return await apiCall<number>("reset_skipped_tickets", { projectId });
  },

  /** Permanently removes a ticket. */
  async deleteTicket(id: number): Promise<void> {
    return await apiCall<void>("delete_ticket", { id });
  },

  /**
   * Diff of everything a run has changed, for the preview pane. A run still in
   * flight is compared against the working tree, so edits show before they are
   * committed.
   */
  async workflowRunDiff(runId: number): Promise<RunDiff> {
    return await apiCall<RunDiff>("workflow_run_diff", { runId });
  },

  /**
   * One run's persisted state. Progress events only exist while a run is in
   * flight, so this is how a finished or failed run is read back.
   */
  async workflowRun(runId: number): Promise<WorkflowRun> {
    return await apiCall<WorkflowRun>("workflow_run", { runId });
  },

  /**
   * Checks the two conditions that make the runner refuse to start: the path
   * must be a git repository, and its tree must be clean.
   */
  async workflowPreflight(projectPath: string): Promise<Preflight> {
    return await apiCall<Preflight>("workflow_preflight", { projectPath });
  },

  /**
   * Turns a project into a repository a run can branch from, committing
   * whatever the directory already holds. Returns the preflight that now
   * holds, so the caller does not have to assume the blocker cleared.
   */
  async initProjectRepository(projectPath: string): Promise<Preflight> {
    return await apiCall<Preflight>("init_project_repository", { projectPath });
  },

  /**
   * A run's recorded agent output, oldest first. Progress events only exist
   * while a run is in flight; this is how a finished run is read back.
   */
  async workflowRunOutput(runId: number): Promise<string[]> {
    try {
      return await apiCall<string[]>("workflow_run_output", { runId });
    } catch (error) {
      console.error("Failed to read run output:", error);
      return [];
    }
  },

  /**
   * Stops a run that is still going. The run tears down its child processes
   * and commits whatever the agent finished, so stopping costs the attempt but
   * not the work.
   */
  async cancelWorkflow(runId: number): Promise<void> {
    return await apiCall<void>("cancel_workflow", { runId });
  },

  /**
   * Starts the queue when the board is set to run one and nothing is going.
   * A run finishing is not the only moment work appears — importing a sheet
   * onto an idle board leaves a queue with nobody to start it.
   *
   * Returns the run it began, or null when there was nothing to do.
   */
  async resumeQueue(projectPath: string): Promise<number | null> {
    try {
      return await apiCall<number | null>("resume_queue", { projectPath });
    } catch (error) {
      console.error("Failed to resume the queue:", error);
      return null;
    }
  },

  /**
   * Answers the question a run is waiting on, letting it carry on from where
   * it stopped.
   */
  ...
  /**
   * Adds whatever is blocking a run to `.gitignore` and commits it. Only
   * untracked paths, and only when no tracked file has been edited.
   *
   * An agent regenerates a project's dependencies and build output on every
   * run, so a repository that does not ignore them is dirty again the moment
   * one finishes and can never reach a second ticket.
   */
  async ignoreBuildOutput(projectPath: string): Promise<Preflight> {
    return await apiCall<Preflight>("ignore_build_output", { projectPath });
  },

  /**
   * Sets a project's working tree aside so a run can start. Reversible with
   * `git stash pop`; the runner never does this on its own.
   */
  async stashProjectChanges(projectPath: string): Promise<Preflight> {
    return await apiCall<Preflight>("stash_project_changes", { projectPath });
  },

  /**
   * Lists past autonomous runs, newest first.
   */
  async listWorkflowRuns(agentId?: number): Promise<WorkflowRun[]> {
    try {
      return await apiCall<WorkflowRun[]>("list_workflow_runs", { agentId });
    } catch (error) {
