...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // A run lives in a task inside this process, so anything still
            // marked running belongs to a process that is gone. Left alone it
            // holds its ticket in progress and blocks that board's queue.
            if let Err(e) = commands::workflow::reconcile_interrupted_runs(&conn) {
                log::error!("could not close out interrupted runs: {}", e);
            }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::workflow::workflow_run_diff,
            commands::workflow::workflow_preflight,
            commands::workflow::init_project_repository,
            commands::workflow::cancel_workflow,
            commands::workflow::answer_workflow_question,
            commands::workflow::resume_queue,
            commands::workflow::stash_project_changes,
            commands::workflow::ignore_build_output,
            commands::workflow::workflow_run_output,
            // Ticket board
            commands::tickets::board_for_project,
            commands::tickets::update_board_project,
            commands::tickets::list_tickets,
            commands::tickets::board_summary,
            commands::tickets::create_ticket,
            commands::tickets::update_ticket,
            commands::tickets::set_ticket_status,
            commands::tickets::delete_ticket,
            commands::tickets::reset_skipped_tickets,
