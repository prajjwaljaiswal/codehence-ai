# Dotsquares AI (dotsquares-ai) — Design Theme Reference
...
| -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Dark-first**                         | The base `@theme` block _is_ the dark palette. Light is an override class. `<html class="dark">` and `<meta name="color-scheme" content="dark">` ship in `index.html`. |
...
content, preview) so only the _content_ uses GitHub colours.
...
- Secondary text is _always_ `text-muted-foreground` — by far the most-used class in the codebase (~550 occurrences).
...
```css
--radius-sm: 0.25rem /*  4px */ --radius-base: 0.375rem /*  6px */
...
} /* let the window be transparent */
html {
  border-radius: var(--radius-lg);
  overflow: hidden;
  clip-path: inset(0 round var(--radius-lg));
} /* clips fixed/backdrop-filter kids */
body {
  border-radius: var(--radius-lg);
  overflow: hidden;
...
- **Product name:** "Dotsquares AI" (`tauri.conf.json` `productName`, window title, `<title>`); binary name remains `dotsquares-ai`; bundle id `dotsquares-ai.asterisk.so`.
- **Wordmark:** `src/assets/dots-logo.svg` (light ink, for dark themes) and `dots-logo-ink.svg` (dark ink, for the light theme). Swapping is CSS-driven:
  ...
  .theme-light .brand-logo-on-light {
    display: block;
  }
  .theme-light .brand-logo-on-dark {
    display: none;
  }
  ```
...
**`color-scheme: dark`** is set on `html` in CSS _and_ via the meta tag — native form
...
1. **Use semantic tokens, never raw greys.** `bg-card`, `text-muted-foreground`, `border-border` — not `bg-gray-800`. Raw Tailwind hues are only for _status_ meaning (§2.8).
